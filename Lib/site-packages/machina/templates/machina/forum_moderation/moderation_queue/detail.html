...
{% block sub_title %}{% trans "Cola de Moderación" %}{% endblock sub_title %}

{% block content %}
    <div class="row">
        <div class="col-xs-12">
            <h1>{% trans "Cola de Moderación" %}</h1>
        </div>
    </div>
	<div class="alert alert-warning" role="alert">
		<div class="pull-right">
			<a href="{% url 'forum_moderation:disapprove_queued_post' post.pk %}" class="btn btn-warning">{% trans "Desaprobar" %}</a>
			<a href="{% url 'forum_moderation:approve_queued_post' post.pk %}" class="btn btn-success"><b>{% trans "Aprobar" %}</b></a>
		</div>
		<h4>{{ post.subject }}</h4>
		<p class="text-muted">{% if post.is_topic_head %}{% trans "Nuevo Tema" %}{% else %}{% trans "Nueva Publicación" %}{% endif %}</p>
	</div>
    <div class="row">
        <div class="col-xs-12">
                            ...
                            <div class="col-md-12 poll-content-wrapper">
                                ...
                                <div class="max-options-indication">
                                    {% if poll.max_options == 1 %}
                                        {% blocktrans trimmed %}
                                            <b>1</b> La opción debería ser seleccionada
                                        {% endblocktrans %}
                                    {% else %}
                                        {% blocktrans trimmed with max_options=poll.max_options %}
                                            Up to <b>{{ max_options }}</b> Las opciones deberían ser seleccionadas
                                        {% endblocktrans %}
                                    {% endif %}
                                </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2 post-sidebar">
                            {% if post.poster %}
                                ...
                                <div class="posts-count text-muted"><b>{% trans "Publicaciones:" %}</b>&nbsp;{{ post.poster.forum_profile.posts_count }}</div>
                            {% else %}
                                <div class="username"><b>{{ post.username }}</b></div>
                                <div class="username text-muted">{% trans "Usuario Anónimo" %}</div>
                            {% endif %}
                        </div>
                    </div>
                </div>
	        ...
	        <div class="col-xs-12">
	            <div class="panel panel-default topic-review">
	                <div class="panel-heading">
	                    <h3 class="panel-title">{% trans "Revisión de Tema" %}&nbsp;&ndash;&nbsp;{{ post.topic.subject }}</h3>
	                </div>
	                <div class="panel-body">
	                            ...
	                            <div class="col-md-10 post-content-wrapper">
                                    ...
                                    {% spaceless %}
                                      ...
                                      {% if post.poster %}
                                        {% url 'forum_member:profile' post.poster_id as poster_url %}
                                        {% blocktrans trimmed with poster_url=poster_url username=post.poster.username creation_date=post.created %}
                                          Por: <a href="{{ poster_url }}">{{ username }}</a> on {{ creation_date }}
                                        {% endblocktrans %}
                                      {% else %}
                                        {% blocktrans trimmed with poster_username=post.username creation_date=post.created %}
                                          Por: {{ poster_username }} on {{ creation_date }}
                                        {% endblocktrans %}
                                      {% endif %}
                                    {% endspaceless %}
	                                </small></p>
