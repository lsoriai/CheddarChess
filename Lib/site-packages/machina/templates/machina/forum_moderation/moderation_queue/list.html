    ...
    <div class="row">
        <div class="col-xs-6 col-md-4 forum-actions-block">
            {% if paginator.count > 0 %}
                <p class="text-muted">
                    {% blocktrans with paginator.count as topic_length %}{{ topic_length }} publicaciones encontradas{% endblocktrans %}
                </p>
            {% endif %}
        </div>
            ...
            <div class="panel panel-default postmoderationlist">
                <div class="panel-heading">
                    <div class="row panel-row">
                        <div class="col-xs-12 col-md-8 post-name-col">
                            <h3 class="panel-title">{% trans "Cola de moderación" %}</h3>
                        </div>
                        <div class="col-md-2 hidden-xs moderation-col">{% trans "Tema" %}</div>
                        <div class="col-md-2 hidden-xs moderation-col">{% trans "Foro" %}</div>
                    </div>
                </div>
                <div class="panel-body">
                    {% for post in posts %}
                        <div class="row panel-row">
                            <div class="col-xs-12 col-md-8 post-name">
                            ...
                            <div class="hidden-xs col-md-2 topic-name">
                                {% if post.is_topic_head %}
                                    <span class="label label-default">{% trans "Nuevo tema" %}</span>
                                {% else %}
                    ...
                    {% empty %}
                        <div class="row panel-row">
                            <div class="col-md-12">
                                {% trans "No hay publicaciones pendientes de aprobación" %}
                            </div>
                        </div>
                    {% endfor %}
                </div>
