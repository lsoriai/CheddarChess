...
<div class="navbar navbar-default navbar-fixed-top machina-navbar" role="navigation">
  <div class="container">
    <div class="navbar-header">
	  ...
	  <a class="navbar-brand" href="/">FORO del ajedrez</a>
    </div>
      ...
      <div class="col-sm-7 col-md-8 col-lg-5">
        <form class="navbar-form form-inline" role="search" action="{% url 'forum_search:search' %}">
          ...
          <div class="form-group">
            <a href="{% url 'forum_search:search' %}" class="btn btn-link">{% trans "Búsqueda avanzada" %}</a>
          </div>
        </form>
      </div>
  ...
  <div class="row">
    <div class="col-xs-12">
      ...
      {% if not request.user.is_anonymous %}
        <a href="{% url 'forum_member:user_subscriptions' %}" class="btn btn-link"><i class="fa fa-bookmark-o ">&nbsp;</i>{% trans "Subscripciones" %}</a>
        <a href="{% url 'forum_member:user_posts' request.user.id %}" class="btn btn-link"><i class="fa fa-comments-o ">&nbsp;</i>{% trans "Ver mis mensajes" %}</a>
      {% endif %}
      {% get_permission 'can_access_moderation_queue' request.user as can_access_moderation_queue %}
      {% if can_access_moderation_queue %}
        <a href="{% url 'forum_moderation:queue' %}" class="btn btn-link"><i class="fa fa-gavel">&nbsp;</i>{% trans "Cola de Moderación" %}</a>
      {% endif %}
      </div>
    </div>
  </div>
